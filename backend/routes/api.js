...
router.get('/admin/poll-results', requireAdmin, async (req, res) => {
  try {
    ...
    // Kiểm tra xem có items active không
    const activeItems = await db.getAllActiveCategoryItems();
    console.log(`📋 Found ${activeItems.length} active category items`);
    
    const results = await db.getPollResults(filters);
    console.log(`✅ Found ${results.length} poll results`);
    
    // Nếu không có kết quả và có items active, thử tạo kết quả ngay
    if (results.length === 0 && activeItems.length > 0) {
      console.log('⚠️ No poll results found, but active items exist. Triggering generation...');
      // Trigger generation for all active items
      for (const item of activeItems) {
        try {
          await db.autoGeneratePollResult(item);
        } catch (err) {
          console.error(`Error generating result for item ${item.id}:`, err);
        }
      }
      // Fetch again after generation
      const newResults = await db.getPollResults(filters);
      console.log(`✅ After generation, found ${newResults.length} poll results`);
